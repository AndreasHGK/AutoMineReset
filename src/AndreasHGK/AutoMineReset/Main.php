<?php

namespace AndreasHGK\AutoMineReset;

use pocketmine\plugin\PluginBase;
use pocketmine\event\Listener;
use pocketmine\command\ConsoleCommandSender;
use pocketmine\command\CommandExecutor;
use pocketmine\command\CommandSender;
use pocketmine\command\Command;
use pocketmine\scheduler\ClosureTask;
use pocketmine\utils\TextFormat as C;
use pocketmine\utils\Config;
use pocketmine\Player;
use pocketmine\Server;

class Main extends PluginBase{
	
	/** @var bool */
	public $paused = false;
	/** @var int */
	private $sec = 0;
	/** @var bool */
	public $autopaused = false;
	/** @var int */
	private $interval = 600;
	
	public function onLoad(){
		Server::getInstance()->getLogger()->notice(C::GREEN." Loading...");
		$this->saveDefaultConfig();
        	$this->reloadConfig();
		if(is_int($this->getConfig()->get('reset-time')) == false){
			if(is_bool($timerseconds > $this->getConfig()->get('sleep-when-empty')) == false){
				Server::getInstance()->getLogger()->notice(C::BOLD.C::RED."[FATAL]".C::RESET.C::YELLOW." Config is setup incorrectly!");
			}
		}

	}
	
	public function prepLoop(){
		$this->sec = 1;
		$this->interval = $this->getConfig()->get('reset-time');
		$this->milliseconds = 1000;
		$h = $this->getScheduler()->scheduleRepeatingTask(new ClosureTask(function($_) : void {
			static $y = false;
			if($this->getConfig()->get('sleep-when-empty') == true){
				if(count(Server::getInstance()->getOnlinePlayers()) != 0){
					if($this->paused == true){
						if($y == true){
							$this->paused = false;
							$this->autopaused = false;
							$y = false;
							Server::getInstance()->getLogger()->notice(C::GREEN." The timer has been auto-enabled!");
						}
					}
				} else {
					if($y == false) {
						$this->paused = true;
						$y = true;
						$this->autopaused = true;
						Server::getInstance()->getLogger()->notice(C::GREEN." The timer has been auto-disabled!");
					}
				}
			}
			if($this->paused == false){
				$this->sec++;
			}
			if($this->sec >= $this->interval){
				$this->resetAll();
				$this->sec = 1;
			}	
		}), 20);
	}
	
	public function onEnable(){
		$this->getLogger()->notice(C::GREEN." Enabled!");
		$this->prepLoop();
	}
	
	public function onCommand(CommandSender $sender, Command $cmd, string $label, array $args) : bool{
		switch(strtolower($cmd->getName())){
			case "mr";
			if($sender->hasPermission("minereset.command.resetall")){
				$this->resetAll();
				return true;
			}
			else{
            			$sender->sendMessage(C::RED . "You do not have permission to run this command." . C::RESET);
				return true;
            		}
			break;
		case "autoreset":
			if($sender->hasPermission("amr.autoreset")){
				if($this->paused == true){
					$this->paused = false;
					$sender->sendMessage(C::BOLD.C::AQUA."»".C::RESET.C::DARK_AQUA." Automatic reset has been ".C::GREEN."enabled".C::DARK_AQUA."!");
					Server::getInstance()->getLogger()->notice(C::GREEN." The timer has been enabled by ".$sender->getName()."!");
				}else{
					$this->paused = true;
					$sender->sendMessage(C::BOLD.C::AQUA."»".C::RESET.C::DARK_AQUA." Automatic reset has been ".C::RED."disabled".C::DARK_AQUA."!");
					Server::getInstance()->getLogger()->notice(C::GREEN." The timer has been disabled by ".$sender->getName()."!");
					$this->autopaused = false;
				}
			} else {
            			$sender->sendMessage(C::RED . "You do not have permission to run this command." . C::RESET);
			}
			return true;
			break;
		}
	}
	
	public function resetAll(){
		Server::getInstance()->dispatchCommand(new ConsoleCommandSender(),"mine reset-all");
		Server::getInstance()->broadcastMessage(C::BOLD.C::AQUA."»".C::RESET.C::DARK_AQUA." All mines have been reset!");
	}
	
	public function onDisable(){
		Server::getInstance()->getLogger()->notice(C::GREEN." Disabled!");
	}
	
}
